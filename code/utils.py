...
def save_checkpoint(state, is_best, checkpoint_dir, name = strftime("%Y-%m-%d-%H-%M-%S", gmtime()), logger=None):
    ...
    last_file_path = os.path.join(checkpoint_dir, 'last_checkpoint' + name + '.pytorch')
    log_info(f"Saving last checkpoint to '{last_file_path}'")
    torch.save(state, last_file_path)
    if is_best:
        best_file_path = os.path.join(checkpoint_dir, 'best_checkpoint' + name + '.pytorch')
